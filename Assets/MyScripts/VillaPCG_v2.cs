...
public class  VillaPCG_v2 : MonoBehaviour
{
    ...
    public bool enableAsymmetry = true;
    public float sideJitter = 1.5f;
    public float sizeJitter = 1.5f;
    public bool randomizeOptionalLinks = true;
    ...
    {
        // ----------------------------
        // Main route: relatively stable
        // ----------------------------
        float lobbyW = Random.Range(8.0f, 10.5f);
        float lobbyH = Random.Range(5.5f, 7.0f);

        float hallW = Random.Range(7.5f, 10.0f);
        float hallH = Random.Range(7.0f, 9.0f);

        float targetW = Random.Range(7.0f, 9.5f);
        float targetH = Random.Range(4.5f, 6.5f);

        Room courtyard = AddRoom(
            "Courtyard / Entrance",
            new Vector2(Random.Range(-0.8f, 0.8f), -9f),
            new Vector2(Random.Range(9f, 11f), Random.Range(4.5f, 5.5f)),
            SecurityLevel.Public
        );

        Room lobby = AddRoom(
            "Lobby",
            new Vector2(Random.Range(-0.8f, 0.8f), -3f),
            new Vector2(lobbyW, lobbyH),
            SecurityLevel.Public
        );

        Room mainHall = AddRoom(
            "Main Hall",
            new Vector2(Random.Range(-0.8f, 0.8f), 4f),
            new Vector2(hallW, hallH),
            SecurityLevel.SemiRestricted
        );

        Room targetRoom = AddRoom(
            "Target Room",
            new Vector2(Random.Range(-0.8f, 0.8f), 12f),
            new Vector2(targetW, targetH),
            SecurityLevel.Critical
        );

        // ----------------------------
        // Left side: independent randomization
        // ----------------------------
        Vector2 storagePos = enableAsymmetry
            ? Jitter(new Vector2(-9.5f, -4.5f), sideJitter, 0.8f)
            : new Vector2(-9f, -4f);

        Vector2 westCorridorPos = enableAsymmetry
            ? Jitter(new Vector2(-10.0f, 0.5f), sideJitter, 1.2f)
            : new Vector2(-9f, 0f);

        Vector2 westWingPos = enableAsymmetry
            ? Jitter(new Vector2(-9.0f, 5.0f), sideJitter, 1.0f)
            : new Vector2(-9f, 5f);

        Vector2 guardPos = enableAsymmetry
            ? Jitter(new Vector2(-9.5f, 11.0f), sideJitter, 1.0f)
            : new Vector2(-9f, 11f);

        Room storage = AddRoom(
            "Storage",
            storagePos,
            new Vector2(
                Random.Range(4.5f, 6.5f),
                Random.Range(4.0f, 5.5f)
            ),
            SecurityLevel.Public
        );

        Room westCorridor = AddRoom(
            "West Service Corridor",
            westCorridorPos,
            new Vector2(
                Random.Range(2.5f, 4.0f),
                Random.Range(7.0f, 10.0f)
            ),
            SecurityLevel.SemiRestricted
        );

        Room westWing = AddRoom(
            "West Wing",
            westWingPos,
            new Vector2(
                Random.Range(5.0f, 7.5f),
                Random.Range(5.0f, 7.0f)
            ),
            SecurityLevel.SemiRestricted
        );

        Room guardRoom = AddRoom(
            "Guard Room",
            guardPos,
            new Vector2(
                Random.Range(5.0f, 7.0f),
                Random.Range(4.5f, 6.0f)
            ),
            SecurityLevel.Restricted
        );

        // ----------------------------
        // Right side: independent randomization
        // ----------------------------
        Vector2 kitchenPos = enableAsymmetry
            ? Jitter(new Vector2(9.5f, -3.8f), sideJitter, 0.8f)
            : new Vector2(9f, -4f);

        Vector2 eastCorridorPos = enableAsymmetry
            ? Jitter(new Vector2(10.2f, 0.0f), sideJitter, 1.2f)
            : new Vector2(9f, 0f);

        Vector2 eastWingPos = enableAsymmetry
            ? Jitter(new Vector2(9.0f, 4.5f), sideJitter, 1.0f)
            : new Vector2(9f, 5f);

        Vector2 safePos = enableAsymmetry
            ? Jitter(new Vector2(9.8f, 10.8f), sideJitter, 1.0f)
            : new Vector2(9f, 11f);

        Room kitchen = AddRoom(
            "Kitchen",
            kitchenPos,
            new Vector2(
                Random.Range(4.5f, 6.8f),
                Random.Range(4.0f, 5.8f)
            ),
            SecurityLevel.Public
        );

        Room eastCorridor = AddRoom(
            "East Service Corridor",
            eastCorridorPos,
            new Vector2(
                Random.Range(2.5f, 4.2f),
                Random.Range(7.0f, 10.5f)
            ),
            SecurityLevel.SemiRestricted
        );

        Room eastWing = AddRoom(
            "East Wing",
            eastWingPos,
            new Vector2(
                Random.Range(5.0f, 7.8f),
                Random.Range(5.0f, 7.2f)
            ),
            SecurityLevel.SemiRestricted
        );

        Room safeRoom = AddRoom(
            "Safe Room / Exit",
            safePos,
            new Vector2(
                Random.Range(5.0f, 7.2f),
        ...
        ConnectRooms(storage, westCorridor, Side.North, Side.South);
        ConnectRooms(westCorridor, westWing, Side.North, Side.South);
        ...
        ConnectRooms(kitchen, eastCorridor, Side.North, Side.South);
        ConnectRooms(eastCorridor, eastWing, Side.North, Side.South);
        ...
        // ----------------------------
        bool addWestHallLink = !randomizeOptionalLinks || Random.value > 0.25f;
        bool addEastHallLink = !randomizeOptionalLinks || Random.value > 0.25f;
        bool addSideShortcut = randomizeOptionalLinks && Random.value > 0.55f;

        if (addWestHallLink)
            ConnectRooms(westWing, mainHall, Side.East, Side.West);

        if (addEastHallLink)
            ConnectRooms(eastWing, mainHall, Side.West, Side.East);

        // 可選：左右側額外捷徑，讓 graph 更不像鏡像樹
        if (addSideShortcut)
            ConnectRooms(westWing, eastWing, Side.East, Side.West);
    }
    ...
    Vector2 Jitter(Vector2 basePos, float jitterX, float jitterY)
    {
        return new Vector2(
            basePos.x + Random.Range(-jitterX, jitterX),
            basePos.y + Random.Range(-jitterY, jitterY)
        );
    }
}